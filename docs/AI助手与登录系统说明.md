...
│   └── feedback.py     # 反馈提交请求体
├── services/
│   ├── auth_service.py      # 注册/登录/会话签发校验删除、bcrypt、is_admin、校验规则
...
└── api/routes/
    ...
    └── admin.py        # /admin/feedback /admin/logs（仅 root 管理员只读）
```

仅以下现有文件是**小增量改动**：`core/config.py`（加会话/管理员配置）、`api/routes/__init__.py`（挂路由）、`requirements.txt`（加 SQLAlchemy、pymysql）、`.env.example`。
...
├── AdminConsole.vue        # 管理员只读查看页（路由 /admin，新标签页打开）
...
> 主应用侧改动仅两处：`App.vue` 挂悬浮组件、`router/index.js` 加 `/admin` 路由（懒加载到本模块的 `AdminConsole.vue`）；另 `api/feedback.js` 改为提交主后端 `/api/feedback`。
...
| 方法 | 路径 | 鉴权 | 返回 `data` |
|---|---|---|---|
| POST | `/api/auth/register` | 否 | `{user:{id,account,expires_at,is_admin}}`，并下发会话 Cookie |
| POST | `/api/auth/login` | 否 | 同上 |
| GET | `/api/auth/me` | Cookie | `{user:{id,account,expires_at,is_admin}}`，未登录/失效 401 |
...
| GET | `/api/admin/feedback` | 管理员 | `{items:[...]}`，非管理员 403、未登录 401 |
| GET | `/api/admin/logs` | 管理员 | `{items:[...]}`，账号活动日志 |

统一响应壳 `{code,message,data}`，成功 `code=0`。错误码：`4001` 入参、`4010/4012/4013` 未登录/过期/无效、`4011` 账号或密码错、`4030` 非管理员、`4090` 账号已注册、`503x` 数据库不可用。
...
# 管理员账号（11 位，逗号分隔）。这些账号登录后即 root，可在 AI 面板看到「反馈系统」入口
ADMIN_ACCOUNTS=
```
...
- **root 管理员**：在 `backend/.env` 的 `ADMIN_ACCOUNTS` 里填 11 位账号（逗号分隔）。该账号正常注册登录后即管理员，`/auth/me` 返回 `is_admin: true`。
- **查看入口**：管理员登录后，AI 对话窗左上角「账号主页」里出现**「反馈系统（管理员）」**按钮 → 点击 `window.open('/admin')` **新标签页**打开只读查看页。
- **查看页 `/admin`**（`AdminConsole.vue`）：两个标签页——**反馈** 与 **账号活动日志**，纯只读。后端 `/api/admin/*` 用会话 Cookie 校验「已登录 + 是管理员」，非管理员 403。
- **日志**：`logs` 表记录 register/login/logout（时间、账号、IP、UA），由 `auth.py` 路由在登录流程里**尽力而为**写入（写失败只告警，不影响登录）。

> 管理页是 SPA 路由，新标签页凭**同一浏览器的会话 Cookie**自动鉴权，无需在标签页间传任何东西。
