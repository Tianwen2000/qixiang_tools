...
- 数据策略：`普通工具尽量不依赖数据库；登录、反馈、后台日志和工具使用日志使用 MySQL`
...
的工具站工程（普通工具尽量保持轻依赖；登录、反馈、后台日志和工具使用日志使用 MySQL）。
...
4. 在普通工具尽量不强依赖数据库的前提下，保持站点可维护、可扩展、可验证
...
1. 普通工具默认不强制依赖数据库，后台、登录和日志能力按需使用 MySQL
2. 工具元信息由配置文件驱动
3. 路由层尽量轻
4. 业务逻辑尽量沉到工具模块
5. 文件请求结束后及时清理临时目录
...
大多数工具本身不依赖数据库；登录、反馈、账号活动日志和工具使用日志使用 MySQL（见 [AI 助手与登录系统说明](./AI助手与登录系统说明.md) 和 [数据库和数据表说明](../数据库和数据表说明.md)）。除此之外并非完全无状态。
...
当前反馈体系采用主后端 MySQL 落库，并在独立后台管理系统里查看。
...
- `/support`
- `POST /api/feedback`
- `GET /api/backoffice/feedback`
- `feedback` 数据表
- 独立后台只读查看页

详细整理请看：

- `docs/反馈和日志相关说明/反馈遗留事项整理.md`
- `docs/数据库和数据表说明.md`
...
- `docs/README.md`
- `docs/技术架构和代码功能说明/前后端架构与联动原理.md`
- `docs/新增小工具开发指南.md`
- `docs/数据库和数据表说明.md`
- `docs/反馈和日志相关说明/日志说明.md`
