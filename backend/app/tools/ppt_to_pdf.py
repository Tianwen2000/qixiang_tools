...
from app.tools.docx_to_pdf import find_libreoffice_executable
...
def _convert_with_libreoffice(input_path: str, target: Path) -> None:
    ...
    if not executable:
        raise AppException(
            message="未安装 LibreOffice，无法进行高保真 PPT 转 PDF",
            code=5002,
            status_code=503,
        )
    ...
    with tempfile.TemporaryDirectory(prefix="qixiang-libreoffice-") as profile_dir:
        ...
        try:
            ...
            raise AppException(message="PPT 转 PDF 超时", code=5002, status_code=504) from exc
...
def run(input_path: str, output_dir: str, **_: dict) -> str:
    target = Path(output_dir) / "ppt-to-pdf.pdf"
